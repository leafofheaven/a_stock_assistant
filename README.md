...
全市场批量补数据：Streamlit 的“数据更新状态”页提供“全市场批量补数据”控制区，可选择本次计划处理数量、每批大小、回看天数和最大重试次数。更新前会先做 DuckDB 锁检测、Python 代理提示和东方财富 K 线接口预检；预检失败不会启动批量更新。命令行也可使用：
...
页面中的“本次未处理数量”表示 full 股票池中本轮未纳入计划的股票，不代表永久跳过。
