...
def _print_text(status: dict[str, Any], db_path: Path) -> None:
    ...
    print(f"- core_price_data_usable: {bool(status.get('core_price_data_usable'))}")
    print(f"- technical_research_usable: {bool(status.get('technical_research_usable'))}")
    print(f"- enhanced_data_usable: {bool(status.get('enhanced_data_usable'))}")
    print(f"- valuation_data_usable: {bool(status.get('valuation_data_usable'))}")
    print(f"- adj_factor_required: {bool(status.get('adj_factor_required'))}")
    print(f"- price_adjustment_status: {status.get('price_adjustment_status') or '暂无'}")
    print(f"- formal_full_market_result_usable: {bool(status.get('formal_full_market_result_usable'))}")
    print(f"- adj_factor_source: {status.get('adj_factor_source') or '暂无'}")
    print(f"- adj_factor_user_note: {status.get('adj_factor_user_note') or '暂无'}")
