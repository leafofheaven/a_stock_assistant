...
def build_tradeable_universe(
    ...
    for stock in stock_basic.to_dict("records"):
        ...
        if _listed_less_than_days(list_date, trade_date, days=120):
        ...
        if _has_severe_financial_missing(stock_basic_window):
...
def _listed_less_than_days(list_date: str, trade_date: str, days: int) -> bool:
...
def _has_severe_financial_missing(daily_basic_window: pd.DataFrame) -> bool:
